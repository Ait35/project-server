...
export const delete_config = async(req: Request , res : Response)=>{
    try{
        ...
        try {
            ...
            await connection.commit(); // เซฟลง Database
            connection.release();      // คืน Connection ให้ระบบ
            ...
            // ถ้า SQL พังกลางทาง (เช่น ชื่อคอลัมน์ผิด) ให้กระโดดมาที่นี่
            await connection.rollback();
            connection.release();
